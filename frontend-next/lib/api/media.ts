...
import { serverApi } from "@/lib/http/api-mutator";

// ============================================================================
// 1. URL Builders
// (Use these in your UI components: <img src={getRecipeImageUrl(...)} />)
// ============================================================================
...
export const getRecipeSmallImageUrl = (
  recipeId: string,
  version: string = "",
  key: string | number = 1
) =>
...
export const getRecipeTinyImageUrl = (
  recipeId: string,
  version: string = "",
  key: string | number = 1
) =>
...
export const getUserProfileImageUrl = (
  userId: string,
  cacheKey: string | number = 1
) =>
  `/api/media/users/${encodeURIComponent(
    userId
  )}/profile.webp?cacheKey=${cacheKey}`;

export const fetchRecipeImageBlob = async (
  recipeId: string,
  version: string = "",
  key: string | number = 1
) => {
  const url = getRecipeImageUrl(recipeId, version, key);

  return serverApi<Blob>({
    url,
    method: "GET",
    // responseType: "blob",
  });
...
export const fetchRecipeSmallImageBlob = async (
  recipeId: string,
  version: string = "",
  key: string | number = 1
) => {
  const url = getRecipeSmallImageUrl(recipeId, version, key);
