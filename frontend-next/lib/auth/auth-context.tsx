...
} from "../api/generated/users-authentication/users-authentication";
import { getLoggedInUserApiUsersSelfGet } from "../api/generated/users-crud/users-crud";
import serverApi from "../http/api-mutator";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const getSession = useCallback(async () => {
    ...
    setStatus("loading");
    try {
      const userData = await getLoggedInUserApiUsersSelfGet();
  ...
  const signIn = useCallback(
    async (credentials: CredentialsRequest) => {
      setStatus("loading");
      try {
        await getTokenApiAuthTokenPost(credentials);
        await getSession();
      } catch (error) {
        setStatus("unauthenticated");
        throw error;
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      await refreshTokenApiAuthRefreshGet();
      await getSession();
    } catch (error) {
      console.error("Token refresh failed:", error);
      throw error;
  ...
  const oauthSignIn = useCallback(async () => {
    try {
      // Extract OAuth callback parameters from URL (code, state, etc.)
      const urlParams = new URLSearchParams(window.location.search);
      const params: Record<string, string> = {};

      // Convert URLSearchParams to plain object for serverApi
      urlParams.forEach((value, key) => {
        params[key] = value;
      });

      // Directly use serverApi to pass the query parameters
      // This is necessary because the generated function doesn't accept params
      await serverApi<unknown>({
        url: "/api/auth/oauth/callback",
        method: "GET",
        params, // Pass OAuth callback params (code, state, etc.)
      });
